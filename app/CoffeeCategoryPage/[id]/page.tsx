...
// Dispatch event when favorites are saved
...
// Dispatch event when adding to favorites
const addToFavorites = (product: Product): FavoriteProduct[] => {
  ...
  if (existingIndex === -1) {
    const favoriteProduct: FavoriteProduct = {
      ...
      originalPrice: product.originalPrice > 0 && product.originalPrice !== 1 ? product.originalPrice : undefined,
      image: product.image,
      brand: product.brand,
...
// Dispatch event when removing from favorites
...
// Clean features array by removing brackets and extra characters
const cleanFeatures = (features: string[]): string[] => {
  return features.map(feature => {
    // Remove [ and ] characters and trim
...
export default function ProductDetailPage() {
  ...
  const productId = params?.id ? (Array.isArray(params.id) ? params.id[0] : params.id) : undefined;
  ...
  useEffect(() => {
    ...
    async function loadProductsOnce() {
      try {
        ...
        const current = allProducts.find(p => String(p._id) === String(productId) || String(p.id) === String(productId)) ?? null;
        setProduct(current);
        ...
        if (current) {
          ...
          const related = allProducts
            .filter(p => {
              ...
              if (String(p._id) === String(current._id) || String(p.id) === String(current._id)) {
                return false;
              }
              // Show products from same category or recommended products
              return p.category?._id === current.category?._id || p.recommended;
            })
            .slice(0, 4);
  ...
  const calculateAverageRating = () => {
    if (comments.length === 0) return product?.rating || 0;
  ...
  // Calculate display price and discount properly
  const displayPrice = product ? (product.priceAfterDiscount ?? product.price) : 0;
  const displayOriginalPrice = product && product.originalPrice > displayPrice && product.originalPrice > 1 ? product.originalPrice : undefined;
  const discountPercentage = displayOriginalPrice && displayOriginalPrice > displayPrice 
    ? Math.round(((displayOriginalPrice - displayPrice) / displayOriginalPrice) * 100) 
    : 0;

  // Get cleaned features
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="flex items-center gap-4 mb-4">
                  ...
                  {product.positiveFeature && product.positiveFeature.trim() && (
              ...
              <div className="bg-white rounded-2xl border border-blue-200 p-6">
                ...
                <div className="flex flex-wrap gap-4 text-sm text-gray-600 mb-6 font-[var(--font-yekan)]">
                  ...
                  {product.totalCount > 0 && (
                    <span>تعداد کل: {product.totalCount} عدد</span>
                  )}
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                  ...
                  {product.hasWarranty && product.warrantyDuration && product.warrantyDuration > 0 && (
        ...
        >
          ...
          <div className="p-6">
            ...
            {activeTab === 'features' && cleanedFeatures.length > 0 && (
            ...
            {activeTab === 'features' && cleanedFeatures.length === 0 && (
              <p className="text-gray-500 text-center py-8 font-[var(--font-yekan)]">
            ...
            {activeTab === 'specifications' && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {product.status && (
                  <div className="flex justify-between py-2 border-b border-blue-100">
                    <span className="text-gray-600 font-[var(--font-yekan)]">وضعیت:</span>
                    <span className="text-gray-800 font-[var(--font-yekan)]">{product.status === 'active' ? 'فعال' : 'غیرفعال'}</span>
                  </div>
                )}
              </div>
            )}
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {relatedProducts.map((p) => {
                const relatedDisplayPrice = p.priceAfterDiscount ?? p.price;
                const relatedDisplayOriginalPrice = p.originalPrice > relatedDisplayPrice && p.originalPrice > 1 ? p.originalPrice : undefined;
                const relatedDiscountPercentage = relatedDisplayOriginalPrice && relatedDisplayOriginalPrice > relatedDisplayPrice 
                  ? Math.round(((relatedDisplayOriginalPrice - relatedDisplayPrice) / relatedDisplayOriginalPrice) * 100) 
                  : 0;
                      ...
                      <div className="relative h-48 w-full rounded-xl overflow-hidden mb-3 bg-gradient-to-br from-blue-100 to-blue-200 flex items-center justify-center">
                        ...
                        {relatedDiscountPercentage > 0 && (
                          <div className="absolute top-2 right-2 bg-orange-500 text-white text-xs px-2 py-1 rounded-full font-bold">
                            {relatedDiscountPercentage}% تخفیف
                          </div>
                        )}
                      </div>
