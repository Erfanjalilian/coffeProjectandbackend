// app/coffee-category/client-page.tsx
"use client";
...
// Custom hook for data fetching - UPDATED
function useProducts(initialProducts: Product[]) {
  const [allProducts, setAllProducts] = useState<Product[]>(initialProducts);
  const [loading, setLoading] = useState(initialProducts.length === 0);
  const [hasFetched, setHasFetched] = useState(false);

  useEffect(() => {
    // Only fetch if initialProducts is empty (fallback)
  ...
  }, [initialProducts.length, hasFetched]);

  return { allProducts, loading };
}

// Custom hook for filters - WITH PROPER RETURN TYPE
function useFilteredProducts(allProducts: Product[]): FilteredProductsResult {
...
export default function ClientCoffeeCategoryPage({ initialProducts, error }: PageProps) {
  // Use the updated hook with initial data
  const { allProducts, loading: productsLoading } = useProducts(initialProducts);
  const { 
    filters, 
    activeFilters, 
    setActiveFilters, 
      ...
      <div className="min-h-screen bg-gradient-to-b from-blue-50 to-white pt-34">
        <div className="max-w-7xl mx-auto px-4 py-8">
          <div className="bg-red-50 border border-red-200 rounded-2xl p-6 text-center">
            <p className="text-red-600 font-[var(--font-yekan)] text-lg">{error}</p>
            <button 
              onClick={() => window.location.reload()}
              className="mt-4 bg-blue-600 text-white px-6 py-2 rounded-xl font-[var(--font-yekan)] hover:bg-blue-700"
            >
              تلاش مجدد
            </button>
          </div>
  ...
  // Show skeleton only if we have NO initial data AND still loading
